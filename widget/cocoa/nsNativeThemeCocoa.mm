...
// Avoid allocating excessively large temporary bitmap buffers when drawing
// native controls.
...
void nsNativeThemeCocoa::DrawSourceListSelection(CGContextRef aContext, const CGRect& aRect,
  ...
  if (aSelectionIsActive) {
    ...
    if (aWindowIsActive) {
      fillColor = [NSColor colorWithCalibratedWhite:0.871 alpha:1.0];
    } else {
      fillColor = [NSColor colorWithCalibratedWhite:0.808 alpha:1.0];
    }
  }
  ...
  CGContextSetRGBFillColor(aContext, red, green, blue, alpha);
...
Maybe<nsNativeThemeCocoa::WidgetInfo> nsNativeThemeCocoa::ComputeWidgetInfo(
  ...
  switch (aAppearance) {
    ...
    case StyleAppearance::SpinnerDownbutton: {
        SpinButtonParams params;
        if (elementState.HasAllStates(ElementState::HOVER |
                                      ElementState::ACTIVE)) {
